...
class InLidaSupabaseUser extends BaseAuthUser {
  InLidaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> inLidaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = InLidaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
